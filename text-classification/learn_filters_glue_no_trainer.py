...
from models.ted_deberta_v2 import TEDDebertaV2Config, TEDDebertaV2ForSequenceClassification

# Will error if the minimal version of Transformers is not installed. Remove at your own risks.
check_min_version("4.28.0")

logger = get_logger(__name__)
MODEL_MAPPING.register(TEDDebertaV2Config, TEDDebertaV2ForSequenceClassification)
CONFIG_MAPPING = {
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="dropout ratio for the task-specific classification layer.",
    )
    parser.add_argument(
        "--hidn_dropout",
        type=float,
        default=0.1,
        help="dropout ratio for the hidden layers.",
    )
    parser.add_argument(
        "--att_dropout",
        type=float,
        default=0.1,
        help="dropout ratio for the attention probability.",
    )
    parser.add_argument(
        "--model_type",
        type=str,
...
def main():
    ...
    # in the environment
    accelerator = (
        Accelerator(log_with=args.report_to, mixed_precision=args.mixed_precision, logging_dir=args.output_dir) if args.with_tracking else Accelerator(mixed_precision=args.mixed_precision)
    )
    ...
    config = config_class.from_pretrained(
        ...
        hidden_dropout_prob=args.hidn_dropout,
        attention_probs_dropout_prob=args.att_dropout,
        train_filters=True,
    ...
    optimizer_grouped_parameters = [
        {
            "params": [p for n, p in model.named_parameters() if "filter" in n and not any(nd in n for nd in no_decay)],
            "weight_decay": args.weight_decay,
        },
        {
            "params": [p for n, p in model.named_parameters() if "filter" in n and any(nd in n for nd in no_decay)],
            "weight_decay": 0.0,
        },
    ]
